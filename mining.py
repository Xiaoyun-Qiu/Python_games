...
class Game:
    def __init__(self):
        self.columnlist=[]  #所有的格子，里面是Rectangle
        self.restrec=[]     #未扫过的区域，对象也是Rectangle
        self.minerec=[]    #设定的雷区，对象为Rectangle
        self.usermark=Stack() #用户设定的雷，对象为image
        
        self.all=[]  #未扫过的格子
        self.mine=[] #雷所在的区域，计算机设定
        self.sweptlist=[] #扫过的区域
        self.mark=Stack()   #标记的区域     此处全部用数组来表示
    ...
    def size(self,width,length,bombnum,win):
        ...
        self.middlebutton=Button(self.win,Point(width*1.0+1.2,length*1.0-3.0),1.6,0.8,'Middle')
        ...
        for i in range(self.width):
            ...
            for j in range(self.length):
                ...
                rowlist.append(rec)
                rec.draw(self.win)
            self.columnlist.append(rowlist)  #完成所有格子对象集合的创建,也是界面创建

        for i in range(self.width):
            for j in range(self.length):
                self.all.append([i,j])    #没操作过的格子开始时就是所有格子

        for i in random.sample(self.all,eval(self.bombnum)):
            self.mine.append(i)    #完成雷的设定，此集合为先列后行，跟格子对象是一样的
    def easy(self):
        self.win.close()
        self.__init__()
    ...
    def setbomb(self,q):
        ...
        if [x,y] in self.all and self.state=='playing':   #只能对还没操作过的格子操作
            if [x,y] in self.mine:  
                self.minenum-=1    #操作正确，显示出来
                image=Image(Point(x*1.0+0.5,y*1.0+0.5),'bomb.gif')
                image.draw(self.win)
                self.usermark.push(image)
                self.all.remove([x,y])
                self.mark.push([x,y])
                self.sweptlist.append([x,y]) #依次加入、移出集合
    ...
    def undo(self):
        if self.mark.mylist!=[]:
            ...
            self.minenum+=1
            self.labelnum.setText('mines:'+str(self.minenum))
    def firsthit(self,x,y): #核心算法，如何接受点击并进行判断
        if [x,y] in self.all:
            if [x,y] in self.mine:
                self.fail()
            else:
    ...
    def fail(self):
        ...
        if new.clicked(p):
            ...
            #对fail状态的操作，不含判断，判断在上一个方法中
    def success(self):      #同样是对success状态的操作
